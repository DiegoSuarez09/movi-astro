...
- `public/images/`: `movi-logo.png` y `movi-logo-white.png` (header y footer, 400×131 — 2x del tamaño máximo en que se muestran en pantalla, no subir versiones más grandes), y la foto del hero como `movi-building.webp` — solo webp, sin fallback a PNG (decisión deliberada, no reintroducir un `.png`). `movi-og-image.png` (1200×630, fondo navy + isologo blanco centrado) es la imagen de vista previa al compartir el link (`og:image`/`twitter:image`) — distinta a propósito de `movi-building.webp`, que se usa como `image` en el JSON-LD (`businessSchema`) porque ahí Google espera una foto real del local, no el logo. Favicons (`favicon.ico`, PNGs en varios tamaños, `apple-touch-icon.png`) viven sueltos en `public/`, no en `public/images/`.
- `public/.htaccess`: reglas de servidor (Apache/LiteSpeed) que Hostinger respeta tal cual sobre `dist/`. Contiene la redirección 301 `www.cientificamovi.com.ar` → `cientificamovi.com.ar` (el dominio "www" no aparece como entidad separada en hPanel, así que no se pudo configurar por su herramienta de Redirecciones) y cabeceras `Cache-Control`: 1 año/`immutable` para el CSS/JS con hash de Astro (`/_astro/*`), 30 días para imágenes/favicons sin hash — si algún día se reemplaza una imagen sin cambiarle el nombre, tarda hasta 30 días en verse el cambio para quien ya visitó el sitio; renombrar el archivo si hace falta que se vea al toque.
